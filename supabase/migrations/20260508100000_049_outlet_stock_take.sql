/*
  Outlet physical stock take: sessions + lines, apply via RPC, ledger trail.
*/

-- ---------------------------------------------------------------------------
-- Tables
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.outlet_stock_take_sessions (
...
CREATE TABLE IF NOT EXISTS public.outlet_stock_take_lines (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id uuid NOT NULL REFERENCES public.outlet_stock_take_sessions(id) ON DELETE CASCADE,
  outlet_inventory_id uuid NOT NULL REFERENCES public.outlet_inventory(id) ON DELETE RESTRICT,
  system_qoh_before numeric(14, 4) NOT NULL,
  counted_qty numeric(14, 4) NOT NULL CHECK (counted_qty >= 0),
  variance numeric(14, 4) NOT NULL,
  line_remark text,
  UNIQUE (session_id, outlet_inventory_id)
);

CREATE INDEX IF NOT EXISTS idx_outlet_stock_take_sessions_outlet ON public.outlet_stock_take_sessions(outlet_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_outlet_stock_take_lines_session ON public.outlet_stock_take_lines(session_id);

ALTER TABLE public.outlet_stock_take_sessions ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.outlet_stock_take_lines ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Non-pending users can view outlet stock take sessions"
  ON public.outlet_stock_take_sessions FOR SELECT TO authenticated
  USING (
    EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role != 'pending')
  );

CREATE POLICY "Non-pending users can view outlet stock take lines"
  ON public.outlet_stock_take_lines FOR SELECT TO authenticated
  USING (
    EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role != 'pending')
  );

-- ---------------------------------------------------------------------------
-- post_outlet_stock_take
-- p_lines: [{"outlet_inventory_id":"uuid","counted_qty":n,"line_remark":"..."}]
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Pass 1: validate + lock rows (hold locks until transaction ends)
