/*
  # admin_delete_supply_order: FG outlet match when line.product_batch is null

  For received finished-goods lines, `supply_order_lines.product_batch` can be
  null while `hub_inventory.product_batch` is set. The delete path matched only
  the line column, so lookup failed and errors showed `batch <NULL>`.

  Use COALESCE(line, hub) for outlet_inventory.product_batch matching (aligned
  with how the line was received/credited).
*/
