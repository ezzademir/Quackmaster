/*
  Outlet-to-outlet stock transfers: pending (reserve source) → dispatched (deduct source) → received (credit dest).
*/
...
CREATE TABLE IF NOT EXISTS public.outlet_transfer_lines (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  outlet_transfer_id uuid NOT NULL REFERENCES public.outlet_transfers(id) ON DELETE CASCADE,
  source_outlet_inventory_id uuid NOT NULL REFERENCES public.outlet_inventory(id) ON DELETE RESTRICT,
  product_batch text NOT NULL,
  quantity numeric(14, 4) NOT NULL CHECK (quantity > 0),
  lot_id uuid REFERENCES public.inventory_lots(id) ON DELETE SET NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (outlet_transfer_id, source_outlet_inventory_id)
);

CREATE INDEX IF NOT EXISTS idx_outlet_transfers_from ON public.outlet_transfers(from_outlet_id);
CREATE INDEX IF NOT EXISTS idx_outlet_transfers_to ON public.outlet_transfers(to_outlet_id);
CREATE INDEX IF NOT EXISTS idx_outlet_transfers_status ON public.outlet_transfers(status);
CREATE INDEX IF NOT EXISTS idx_outlet_transfer_lines_transfer ON public.outlet_transfer_lines(outlet_transfer_id);

ALTER TABLE public.outlet_transfers ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.outlet_transfer_lines ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can view outlet_transfers"
  ON public.outlet_transfers FOR SELECT TO authenticated USING (true);

CREATE POLICY "Authenticated users can insert outlet_transfers"
  ON public.outlet_transfers FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY "Authenticated users can update outlet_transfers"
  ON public.outlet_transfers FOR UPDATE TO authenticated USING (true) WITH CHECK (true);

CREATE POLICY "Authenticated users can view outlet_transfer_lines"
  ON public.outlet_transfer_lines FOR SELECT TO authenticated USING (true);

CREATE POLICY "Authenticated users can insert outlet_transfer_lines"
  ON public.outlet_transfer_lines FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY "Authenticated users can update outlet_transfer_lines"
  ON public.outlet_transfer_lines FOR UPDATE TO authenticated USING (true) WITH CHECK (true);

-- ---------------------------------------------------------------------------
-- create_outlet_transfer
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- receive_outlet_transfer (credit destination; merge by outlet + batch + lot)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- cancel_outlet_transfer (pending: release reserve; dispatched: restore source qoh)
-- ---------------------------------------------------------------------------
