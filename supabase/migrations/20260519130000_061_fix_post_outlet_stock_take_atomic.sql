/*
  Fix post_outlet_stock_take regression from 057_movement_rpc_instrumentation:

  Migration 057 dropped the validate-then-write pass and the counted_below_reserved
  guard while adding movement-context instrumentation. Soft RETURN after partial
  UPDATEs commits (PostgREST does not roll back on jsonb success:false), which can
  corrupt outlet_inventory / movements / stock-take sessions mid-batch.

  Restore 051's two-pass validation (including reserved + duplicate checks) while
  keeping movement ctx + post_stock_take snapshot hook from 056/057/059.
*/
...
BEGIN
  ...
  -- Pass 1: validate + lock rows (hold locks until transaction ends). No writes yet.
  ...
  LOOP
    ...
    IF v_oi_id = ANY(seen_ids) THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', 'duplicate_outlet_inventory_id',
  ...
  -- Pass 2: apply counts (rows already locked + validated)
  FOR rec IN SELECT * FROM jsonb_array_elements(p_lines)
  LOOP
    v_oi_id := NULLIF(trim(both from rec->>'outlet_inventory_id'), '')::uuid;
    v_counted := NULLIF(rec->>'counted_qty', '')::numeric;
  ...
  PERFORM public._post_stock_take_snapshot_hook(
    p_outlet_id,
    COALESCE(p_count_date, CURRENT_DATE)
  );
