/*
  # admin_delete_supply_order: received + RM outlet rows

  supply_order_lines.product_batch can be NULL for raw-material hub lines (052).
  The previous lookup used `oi.product_batch = rec.product_batch`, which never
  matches when both are NULL. Join hub_inventory and match outlet_inventory by
  raw_material_id when the hub line is an RM row.
*/
