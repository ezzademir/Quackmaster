...
const MAX_TXNS = 5000;
/** When false, POS tickets are compare-only (POS compare). Outlet Sales stays manual. */
  ...
  try {
    ...
    if (action === "sync") {
      if (!INGEST_ENABLED) {
        return jsonResponse({
          error:
            "Journal ingest is turned off. Key sales in Outlet Sales (manual). Use POS compare to compare POS tickets to those journals.",
        }, req, 400);
      }
      const admin = makeAdminClient();
